    ...
    <KeyboardAvoidingView style={[styles.container, { backgroundColor: theme.surface }]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={styles.content}>
        ...
        <View style={styles.footer}>
          <Text style={[styles.footerText, { color: theme.grey }]}>Don't have a doctor account? </Text>
          <Pressable onPress={() => navigation.navigate('DoctorRegister')}>
            <Text style={[styles.footerLink, { color: theme.primary }]}>Register</Text>
          </Pressable>
        </View>

        <Pressable style={styles.patientLink} onPress={() => navigation.navigate('Login')}>
          <Ionicons name="person-outline" size={16} color={theme.secondary} />
          <Text style={[styles.patientLinkText, { color: theme.secondary }]}>Are you a patient? Sign in here</Text>
        </Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  patientLink: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: spacing.lg, gap: spacing.xs },
  patientLinkText: { ...typography.bodySmall, fontWeight: '500' },
});
