...
const LoginScreen = ({ navigation }) => {
  ...
  const showAlert = (title, message, icon = 'alert-circle', iconColor = theme.warning) => setAlert({ visible: true, title, message, icon, iconColor });
  const hideAlert = () => setAlert({ ...alert, visible: false });

  const handleLogin = async () => {
    if (!email || !password) { showAlert('Missing Fields', 'Please fill in all fields.', 'information-circle', PATIENT_COLOR); return; }
    setLoading(true);
    try {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.form}>
          ...
          <View style={[styles.inputContainer, { backgroundColor: theme.inputBg }]}>
            ...
            <Pressable onPress={() => setShowPassword(!showPassword)}><Ionicons name={showPassword ? 'eye-outline' : 'eye-off-outline'} size={20} color={theme.grey} /></Pressable>
          </View>
          <Pressable style={[styles.button, { backgroundColor: PATIENT_COLOR }, loading && { opacity: 0.7 }]} onPress={handleLogin} disabled={loading}>
            {loading ? <ActivityIndicator color="#FFF" /> : <Text style={styles.buttonText}>Sign In</Text>}
          </Pressable>
        </View>
        <View style={styles.footer}>
          <Text style={[styles.footerText, { color: theme.grey }]}>Don't have an account? </Text>
          <Pressable onPress={() => navigation.navigate('Register')}><Text style={[styles.footerLink, { color: PATIENT_COLOR }]}>Sign Up</Text></Pressable>
        </View>
        <Pressable style={styles.doctorLink} onPress={() => navigation.navigate('DoctorLogin')}>
          <Ionicons name="medkit-outline" size={16} color={theme.secondary} />
          <Text style={[styles.doctorLinkText, { color: theme.secondary }]}>Are you a doctor? Join as a provider</Text>
        </Pressable>
      </View>
...
const styles = StyleSheet.create({
  container: { flex: 1 }, content: { flex: 1, justifyContent: 'center', paddingHorizontal: spacing.xl },
  ...
  doctorLink: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: spacing.lg, gap: spacing.xs },
  doctorLinkText: { ...typography.bodySmall, fontWeight: '500' },
});
