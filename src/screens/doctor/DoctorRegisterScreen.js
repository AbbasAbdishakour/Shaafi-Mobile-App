  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (authError) {
        ...
        if (signInError) throw new Error('Wrong password for existing account.');
        userId = signInData.user.id;
      } else {
        userId = authData.user.id;
      }

      // Step 2: Insert doctor profile
      const { error: insertError } = await supabase.from('doctors').insert({
        user_id: userId,
        name, specialty, hospital,
        experience: parseInt(experience) || 0,
        price_per_session: parseInt(price) || 50,
      ...
      showAlert('Registration Submitted!', 'Your profile is under review.', 'checkmark-circle', theme.success, () => navigation.goBack());
    } catch (error) {
    ...
    <KeyboardAvoidingView style={[styles.container, { backgroundColor: theme.surface }]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          <View style={styles.iconContainer}><Ionicons name="medkit" size={40} color={theme.primary} /></View>
        ...
        <View style={styles.form}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.specialtyScroll}>
            {specialties.map((s) => (
              <Pressable key={s} style={[styles.specialtyChip, { backgroundColor: specialty === s ? theme.primary : theme.inputBg }]} onPress={() => setSpecialty(s)}>
        ...
        <View style={styles.footer}>
          ...
          <Pressable onPress={() => navigation.goBack()}><Text style={[styles.footerLink, { color: theme.primary }]}>Sign In</Text></Pressable>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  container: { flex: 1 }, content: { flexGrow: 1, paddingHorizontal: spacing.xl, paddingVertical: spacing.xxl },
